...
class GUIWidget():
    def __init__(self, main, experiment, samples, test, port):
        self.main = main
        self.exp = experiment
        self.samp = samples
        self.test = test
    ...
    def expframe(self):
        ...
        if self.test == 0:
            ...
            self.lexp.config(text = self.port)
            self.ph.connectarduino(self.port)
        self.wamplitude(2)
        self.woffset(3)
        self.wgain(4)
        self.emp(self.main, 5)
    
    def wcolor(self, nrow):
        self.color = Tk.IntVar()
        lcolor = Tk.Label(self.lexp, text = "Color:")
        lcolor.grid(row = nrow, column = 0, sticky = Tk.W)
    ...
    def wsamples(self, nrow, ncolumn):
        lsamp = Tk.Label(self.lexp, text = "Samples:")
        lsamp.grid(row = nrow, column = ncolumn, sticky = Tk.W)
        self.esamp = Tk.Entry(self.lexp, width = 6)
        self.esamp.grid(row = nrow, column = ncolumn + 1, sticky = Tk.W)
        self.esamp.insert(0, self.samp)
    ...
    def wamplitude(self, nrow):
        ...
        self.eamp = Tk.Entry(self.lexp, width = 8)
    ...
    def woffset(self, nrow):
        ...
        self.eoff = Tk.Entry(self.lexp, width = 8)
    ...
    def wgain(self, nrow):
        ...
        self.ega = Tk.Entry(self.lexp, width = 8)
        self.ega.grid(row = nrow, column = 4, sticky = "W")
        checkbutton = Tk.Button(self.lexp, text = "Check", command = self.getega)
        checkbutton.grid(row = nrow, column = 5, sticky = Tk.E + Tk.W)
        
    def emp(self, main, nrow):
        self.lxemp = Tk.Label(main, text = "       ")
        self.lyemp = Tk.Label(main, text = "")
    ...
    def getcolor(self):
        ...
        if self.test:
            self.ph.sendcol(self.oldc, col)
            self.oldc = col
        return col
    
    def getsamples(self):
        samp = int(self.esamp.get())
        if samp >= 0:
            self.samples = samp
            return samp
    ...
    def getamplitude(self, a):
        ...
        if self.test:
            self.ph.sendamp(amp, self.color.get())
        return amp
    ...
    def getoffset(self, o):
        ...
        if self.test:
            self.ph.sendoff(off, self.color.get())
        return off
    ...
    def getgain(self, g):
        ...
        if self.test:
            self.ph.sendga(ga, self.color.get())
        return ga
    ...
    def expframegrid(self, row, column):
        ...
        self.lxemp.grid(row = row, column = column + 1)
        self.lyemp.grid(row = row + 1, column = column)
        
    def forget(self):
        self.lexp.grid_remove()
        self.lxemp.grid_remove()
        self.lyemp.grid_remove()
    ...
    def setsamples(self, samples):
        self.esamp.delete(0, Tk.END)
        self.esamp.insert(0, samples)
        self.getsamples()
